...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation through provider dispatch using your configured bug-fix descriptor (default `claude:claude-fable-5-1@max`) with `isolated-write` in a dedicated worktree and a specific scope. Review the diff.
